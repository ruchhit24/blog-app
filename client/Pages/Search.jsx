...
export default function Search() {
  ...
  const handleShowMore = async () => {
    ...
    const searchQuery = urlParams.toString();
    const res = await fetch(`/api/post/gePost?${searchQuery}`);
    if (!res.ok) {
      return;
    }
    if (res.ok) {
  ...
  return (
    <div className='flex flex-col md:flex-row '>
      <div className='p-7 border-b md:border-r md:min-h-screen border-gray-700'>
        <form className='flex flex-col gap-8 justify-between' onSubmit={handleSubmit} style={{gap : '2rem'}}>
      ...
      <div className='w-full'>
        ...
        <div className='p-7 flex flex-wrap gap-4 justify-evenly'>
